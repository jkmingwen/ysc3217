...
int checkSudoku_openmp(int *S)
{
  int *sudoku_fail = (int *) malloc(27 * sizeof(int));
    
#pragma omp parallel shared(sudoku_fail)
  {
	...
	sudoku_fail[check_index] = checkArray_openmp(row);
      }
	...
	sudoku_fail[check_index] = checkArray_openmp(col);
      }
	...
	for (int gy = gx; gy < (gridEnd + 1); gy += 27) /* corner indexes 
							   ...
							   direction */
	  {
	    ...
	    sudoku_fail[check_index]= checkArray_openmp(grid);
	    check_index++;
	  }
      }
  }
