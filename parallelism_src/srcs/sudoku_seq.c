...
int checkSudoku_seq(int *S)
{
	...
	sudoku_fail[check_index] = checkArray_seq(row);
    }
	...
	sudoku_fail[check_index] = checkArray_seq(col);
    }
							   ...
							   direction */
	{
	    ...
	    sudoku_fail[check_index]= checkArray_seq(grid);
	    check_index++;
	}
    }
